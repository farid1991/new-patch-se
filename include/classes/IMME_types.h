...
typedef struct
{
    time_t Hours;        // Number of hours.
    time_t Minutes;      // Number of minutes (0 - 59).
    time_t Seconds;      // Number of seconds (0 - 59).
    time_t MilliSeconds; // Number of milliseconds (0 - 999).
} TMMETime;
...
typedef struct
{
    ...
    uint16_t bpmTempo;                   // Beats-per-minute tempo. Only valid for MIDI and iMelody files.
    uint8_t videoImageSizePresent;       // TRUE if there is available information about VideoImageSize.
    ...
    uint8_t isLiveStreaming;                                                 // Description if the content is live streamig or not.
    uint8_t is_DRM_Protected;                                                // Description if the content is DRM protected or not.
} TMMEMediaProperties;
...
typedef struct MME_DATA
{
    ...
    uint32_t streamingContentId;                                             // Streaming content format identifier.
    uint8_t isLiveStreaming;                                                 // Description if the content is live streamig or not.
    uint8_t is_DRM_Protected;                                                // Description if the content
} MME_DATA;
#else
typedef struct MME_DATA
{
...
typedef struct
{
    ...
    // Audio
    uint8_t audioCodecId;                // Audio codec identifier.
    TMMEAudioCodecIdType audioCodecType; // Audio codec id type description, @see TMMEAudioCodecIdType.
