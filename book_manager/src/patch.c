...
#else
THUMB16
NEWCODE int _ConnectionManager_Connection_GetState()
{
	char buf = 0;
...
THUMB16
NEWCODE void win12512unicode(wchar_t *ws, char *s, int len)
{
	int c;
	while ((c = *s++) && ((len--) > 0))
	{
...
THUMB16
NEWCODE int wcharh2int(wchar_t n)
{
	int r = 0;
	if (n >= '0' && n <= '9')
		r = n - 0x30;
...
THUMB16
NEWCODE void GetIconByName(wchar_t *name, int *imageid)
{
...
THUMB16
NEWCODE int read_ini_file(const wchar_t *filename, char **ini_data)
{
	char *data = NULL;
	int fsize = 0;
	FSTAT _fstat;
...
THUMB16
NEWCODE void Load_booknames(BOOK_MANAGER *data)
{
	char *buffer;
...
THUMB16
NEWCODE void Load_shortcuts(BOOK_MANAGER *data)
{
	char *buffer;
...
THUMB16
NEWCODE int GetActiveTab(BOOK *book)
{
...
THUMB16
NEWCODE BOOK_LIST_ELEM *GetBookListItem(int tab_num)
{
...
THUMB16
NEWCODE BOOK *GetBook(int tab_num)
{
...
THUMB16
NEWCODE BOOL CheckEvent(BOOK *book, int event)
{
	if (book->base_page)
	{
...
THUMB16
NEWCODE void kill_book_and_elf(BOOK *book, GUI *gui)
{
	BOOK_MANAGER *data = GetData();
			...
			else
			{
				if (CheckEvent(selected_book, TERMINATE_SESSION_EVENT))
				{
					UI_Event_toBookID(
					        ...
					        BookObj_GetBookID((BOOK *)List_Get(
					                selected_book->xbook->app_session->listbook,
					                List_GetCount(selected_book->xbook->app_session->listbook) -
					                        1)));
				}
		...
		if (selected_book)
		{
			...
			else
			{
				...
				else
				{
					...
					MessageBox_NoImage(EMPTY_TEXTID,
					                   TextID_Create(buf, ENC_UCS2, MAXELEMS(buf)),
					                   1,
					                   0,
					                   selected_book);
				}
...
THUMB16
NEWCODE void Close_Activitymenu(BOOK *book, GUI *gui)
{
	BOOK_MANAGER *data = GetData();
...
THUMB16
NEWCODE void Exit_to_Standby(BOOK *book, GUI *gui)
{
	BOOK_MANAGER *data = GetData();
	data->tab_pos = GetActiveTab(book);
...
THUMB16
NEWCODE void onEnterPressed_Books(BOOK *book, GUI *gui)
{
...
THUMB16
NEWCODE void minimise_all_books(BOOK *book)
{
...
THUMB16
NEWCODE void close_all_books(BOOK *book)
{
	UI_Event(RETURN_TO_STANDBY_EVENT);
	FreeBook(book);
}
...
THUMB16
NEWCODE void parse_and_run_shorcut(BOOK_MANAGER *data, int key)
{
	if (!data->shortcut_buf)
		return;
...
THUMB16
NEWCODE void myOnKey_Unified(DISP_OBJ *disp_obj, int key, int count, int repeat, int mode)
{
	BOOK_MANAGER *data = GetData();
...
THUMB16
NEWCODE TEXTID Get_BookName(BOOK_MANAGER *data, char *book_name)
{
...
THUMB16
NEWCODE IMAGEID Get_BookIcon(BOOK_MANAGER *data, char *book_name, int iconid)
{
	char *pos;
	char *end_pos;
	wchar_t ws[MAX_BOOK_NAME_LEN];
...
THUMB16
NEWCODE int BookMenu_onMessage(GUI_MESSAGE *msg)
{
	BOOK_MANAGER *data = GetData();
	...
	case LISTMSG_GetItem:
		...
		if (elem->isJava)
			...
			GUIonMessage_SetMenuItemIcon(
			        msg, AlignLeft, Get_BookIcon(data, elem->book_name, BOOK_ITEM_ICON));
...
THUMB16
NEWCODE void CreateBookMenu(BOOK *book, int tab_num)
{
	ActivityMenuBook *mbk = (ActivityMenuBook *)book;
...
THUMB16
NEWCODE void onEnterPressed_Elfs(BOOK *book, GUI *gui)
{
	BOOK *selected_book = GetBook(TAB_ELFS);
	if (selected_book)
	{
...
THUMB16
NEWCODE void onAuthor(BOOK *book, GUI *gui)
{
	BOOK *selected_book = GetBook(TAB_ELFS);
	if (selected_book)
	{
		if (CheckEvent(selected_book, ELF_SHOW_INFO_EVENT))
		{
			...
			msg->book = book;
			UI_Event_toBookIDwData(ELF_SHOW_INFO_EVENT,
			                       BookObj_GetBookID(selected_book),
			                       msg,
			                       (void (*)(void *))mfree_adr());
		}
		else
...
THUMB16
NEWCODE void RefreshElfSoftkeys(BOOK_MANAGER *data)
{
	if (data->elfs_count)
	{
...
THUMB16
NEWCODE int Elf_OpenFile(const wchar_t *filepath, const wchar_t *filename)
{
	W_FSTAT _wstat;
...
THUMB16
NEWCODE void onBcfgConfig(BOOK *book, GUI *lt)
{
	BOOK *selected_book = GetBook(TAB_ELFS);
	if (selected_book)
	{
		if (CheckEvent(selected_book, ELF_BCFG_CONFIG_EVENT))
		{
			...
			if (err == BCFG_SUCCESS)
			{
				...
				UI_Event_toBookIDwData(ELF_BCFG_CONFIG_EVENT,
				                       BookObj_GetBookID(selected_book),
				                       msg,
				                       (void (*)(void *))mfree);
			}
...
THUMB16
NEWCODE int ElfMenu_onMessage(GUI_MESSAGE *msg)
{
	BOOK_MANAGER *data = GetData();
	switch (GUIonMessage_GetMsg(msg))
	{
...
THUMB16
NEWCODE TEXTID GetElfCountTextID(BOOK_MANAGER *data)
{
	wchar_t buf[16];
...
THUMB16
NEWCODE void CreateElfMenu(BOOK *book, int tab_num)
{
	ActivityMenuBook *mbk = (ActivityMenuBook *)book;
...
#ifndef DB2010
THUMB16
NEWCODE void ShortcutMenu_onEnterPressed(BOOK *book, GUI *gui)
{
	BOOK_MANAGER *data = GetData();
...
THUMB16
NEWCODE int ShortcutMenu_onMessage(GUI_MESSAGE *msg)
{
...
THUMB16
NEWCODE void CreateShortcutMenu(BOOK *book, int tab_num)
{
	ActivityMenuBook *mbk = (ActivityMenuBook *)book;
...
THUMB16
NEWCODE int LIST_ELEM_Filter(void *elem)
{
	return (elem ? TRUE : FALSE);
}

THUMB16
NEWCODE void LIST_ELEM_Destroy(void *data)
{
...
THUMB16
NEWCODE void DestroyLists(BOOK_MANAGER *data)
{
	if (data->books_list)
	{
...
THUMB16
NEWCODE void Destroy_ini_Buffer(BOOK_MANAGER *data)
{
	if (data->books_name_buf)
	{
		mfree(data->books_name_buf);
...
THUMB16
NEWCODE void CreateBookAndElfsLists()
{
	BOOK_MANAGER *data = GetData();
	...
	for (int session_index = 0; session_index < total_session; session_index++)
	{
		...
		for (int listbook_index = 0; listbook_index < List_GetCount(session->listbook); listbook_index++)
		{
			...
			if ((total_gui = List_GetCount(book->xguilist->guilist)) ||
			    (((int)book->onClose) & FLASH_MASK) != mask)
			{
#ifdef DB2010
...
#endif
				{
					...
					{
						TextID_GetString(
						        BookObj_GetSession(book)->name, str_tmp, MAXELEMS(str_tmp));
					...
					if (elem->isJava)
					{
						...
						if (!JavaAppDesc_GetFirstApp(JavaDesc))
						{
							...
							{
								wchar_t *name;
								JavaAppDesc_GetJavaAppInfo(
								        JavaDesc, JAVAAPP_NAME, &name);

								if (!wstrncmp(book_name, name, 8))
								{
									wchar_t *fullpath;
									JavaAppDesc_GetJavaAppInfo(
									        JavaDesc, JAVAAPP_FULLPATH, &fullpath);
									JavaApp_LogoImageID_Get(fullpath,
									                        &elem->book_icon);
									mfree(fullpath);
									next = FALSE;
								}
...
THUMB16
NEWCODE void DestroyAllGUIs(BOOK_MANAGER *data)
{
...
THUMB16
NEWCODE void ActivityBook_onClose()
{
	BOOK_MANAGER *data = GetData();
	Destroy_ini_Buffer(data);
	DestroyAllGUIs(data);
...
THUMB16
NEWCODE TEXTID EventsTitleText()
{
	wchar_t buf[32];
