...
THUMB16
NEWCODE inline size_t TO_KBYTE(size_t i)
{
	return i >> 10;
}
THUMB16
NEWCODE inline size_t TO_MBYTE(size_t i)
{
	return i >> 20;
}
THUMB16
NEWCODE inline BOOL is_readonly(int attr)
{
	return ((attr & ~FSX_O_ENCRYPT) & (0x100 >> 1)) ? FALSE : TRUE;
}

THUMB16
NEWCODE BOOL is_directory(const wchar_t *path)
{
	W_FSTAT fs;
...
THUMB16
NEWCODE void read_directory(const wchar_t *fullpath, int *files_count, int *dir_count)
{
...
THUMB16
NEWCODE TEXTID file_count_to_textid(const wchar_t *fullpath, TEXTID space)
{
	W_FSTAT fs;
	w_fstat(fullpath, &fs);
...
THUMB16
NEWCODE TEXTID dir_count_to_textid(const wchar_t *fullpath, TEXTID space)
{
	int files, folders;
...
THUMB16
NEWCODE TEXTID db_set_secondline_text(const wchar_t *fullpath, TEXTID space)
{
