...
THUMB16 NEWCODE BOOL tagid_and_shortdata_to_name(uint16_t tag_id,
                                                 uint16_t data,
                                                 char *name,
                                                 char *value)
{
...
THUMB16 NEWCODE BOOL tagid_and_rationaldata_to_name(uint16_t tag_id,
                                                    uint32_t *data,
                                                    char *name,
                                                    char *value)
{
...
THUMB16 NEWCODE BOOL tagid_to_name(uint16_t tag_id, char *name)
{
    BOOL skip = FALSE;
    ...
    case NANOEXIF_TAG_MAKE:
        strncpy(name, STR_CAMERA_MAKE, MAX_NAME);
        break;

    case NANOEXIF_TAG_MODEL:
        strncpy(name, STR_CAMERA_MODEL, MAX_NAME);
        break;

    case NANOEXIF_TAG_SOFTWARE:
        strncpy(name, STR_SOFTWARE, MAX_NAME);
        break;

    case NANOEXIF_TAG_DATETIME:
        strncpy(name, STR_MODIFY_DATE, MAX_NAME);
        break;

    case NANOEXIF_TAG_COPYRIGHT:
        strncpy(name, STR_COPYRIGHT, MAX_NAME);
        break;

    case NANOEXIF_TAG_DATETIME_ORI:
        strncpy(name, STR_DATETIME_ORIGINAL, MAX_NAME);
        break;

    case NANOEXIF_TAG_LENSMAKE:
        strncpy(name, STR_LENS_MAKE, MAX_NAME);
        break;

    case NANOEXIF_TAG_LENSMODEL:
        strncpy(name, STR_LENS_MODEL, MAX_NAME);
        break;

    case NANOEXIF_TAG_GPS_LATITUDE_REF:
        strncpy(name, STR_GPS_LAT_REF, MAX_NAME);
        break;

    case NANOEXIF_TAG_GPS_LONGITUDE_REF:
        strncpy(name, STR_GPS_LON_REF, MAX_NAME);
        break;

    case NANOEXIF_TAG_GPS_SATELLITES:
        strncpy(name, STR_GPS_SATELLITES, MAX_NAME);
        break;
...
THUMB16 NEWCODE void jpg_read_exif_tag(LIST *list, nanoexif *ne, uint32_t ifd_offset, uint16_t *processed_tags, uint16_t *processed_count)
{
...
THUMB16 NEWCODE LIST *jpeg_read_exif_to_list(const wchar_t *path, const wchar_t *name)
{
...
THUMB16 NEWCODE TEXTID patch_brw_jpg_info(TEXTID text_id, SUB_EXECUTE *sub_exec)
{
