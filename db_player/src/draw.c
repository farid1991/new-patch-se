...
THUMB16 NEWCODE void DrawImageEx(GC *gc, int x, int y, int w, int h, IMAGEID image_id)
{
    if (image_id != NOIMAGE)
    {
...
THUMB16 NEWCODE void DrawImage(int x, int y, IMAGEID image_id)
{
    DrawImageEx(get_DisplayGC(), x, y, 0, 0, image_id);
}

THUMB16 NEWCODE void DrawProgressBar(DBP_DATA *data, int cur_value, int total_value, RECT rect, uint32_t b_color, uint32_t e_color)
{
...
THUMB16 NEWCODE void DrawText(int font, TEXTID text_id, int align, int x1, int y1, int x2, uint32_t t_color)
{
...
THUMB16 NEWCODE void DrawTextEx(int font, TEXTID text_id, int align, int x1, int y1, int x2, int y2, uint32_t t_color)
{
    if (text_id != EMPTY_TEXTID)
    {
#if defined(DB3200) || defined(DB3210) || defined(DB3350)
...
THUMB16 NEWCODE void DrawTextWithStyle(TEXTID text_id, int font, int align, int x1, int y1, int x2, uint32_t t_color, uint32_t h_color, int overlay)
{
