...
THUMB16 NEWCODE const wchar_t *GetCoverType(char cover_type)
{
    switch (cover_type)
    {
    case 0:
...
THUMB16 NEWCODE IMAGEID MetaData_GetCover(DBP_DATA *Data)
{
...
THUMB16 NEWCODE IMAGEID GetCoverArt(DBP_DATA *Data)
{
...
THUMB16 NEWCODE TEXTID GetTags(wchar_t *path, wchar_t *name, int tag_id)
{
    TEXTID text_id;
...
THUMB16 NEWCODE const char *GetFormatType(DBP_DATA *data)
{
...
THUMB16 NEWCODE TEXTID GetFileExt(DBP_DATA *data)
{
    return TextID_Create(GetFormatType(data), ENC_LAT1, strlen(GetFormatType(data)));
}

THUMB16 NEWCODE TEXTID GetTotalTime(DBP_DATA *data)
{
    char buf[16];
...
THUMB16 NEWCODE TEXTID GetElapsedTime(DBP_DATA *data)
{
    char buf[16];
    snprintf(buf, MAXELEMS(buf),
             FMT_TIME1,
...
THUMB16 NEWCODE TEXTID GetRemainingTime(DBP_DATA *data)
{
...
THUMB16 NEWCODE int synchsafeToNormal(uint8_t header[HEADER_SIZE])
{
...
THUMB16 NEWCODE int get_tag_size(wchar_t *path, wchar_t *name)
{
    int ret = 0;
...
THUMB16 NEWCODE TEXTID GetBitrate(DBP_DATA *data)
{
...
THUMB16 NEWCODE const char *getSampleRate(int index)
{
    switch (index)
    {
    case 8000:
...
THUMB16 NEWCODE TEXTID GetSampleRate(DBP_DATA *data)
{
    return TextID_Create(getSampleRate(data->samplerate), ENC_LAT1, strlen(getSampleRate(data->samplerate)));
}

THUMB16 NEWCODE TEXTID GetAudioOutput(DBP_DATA *data)
{
    switch (data->audio_output)
    {
    case TMMEAudioOutput_Mono:
        return TextID_Create(OUT_MONO, ENC_LAT1, 4);
    case TMMEAudioOutput_Stereo:
        return TextID_Create(OUT_STEREO, ENC_LAT1, 6);
    default:
        return TEXT_UNKNOWN;
    }
}
