...
THUMB16 NEWCODE void IncreaseTime(DISP_OBJ_TIMEINPUT *disp_obj, int cursor, int step)
{
...
THUMB16 NEWCODE void DecreaseTime(DISP_OBJ_TIMEINPUT *disp_obj, int cursor, int step)
{
    int *buffer = (cursor == TIME_MIN) ? &disp_obj->buf_min : &disp_obj->buf_sec;
...
THUMB16 NEWCODE void ToggleCursorPosition(DISP_OBJ_TIMEINPUT *disp_obj)
{
    disp_obj->cursor = (disp_obj->cursor == TIME_MIN) ? TIME_SEC : TIME_MIN;
}

THUMB16 NEWCODE int Time_OnCreate(DISP_OBJ *disp)
{
...
THUMB16 NEWCODE void Time_OnClose(DISP_OBJ *disp)
{
    DISP_OBJ_TIMEINPUT *disp_obj =(DISP_OBJ_TIMEINPUT *)disp;
...
THUMB16 NEWCODE void Time_OnRedraw(DISP_OBJ *disp, int r1, int r2, int r3)
{
    DISP_OBJ_TIMEINPUT *disp_obj =(DISP_OBJ_TIMEINPUT *)disp;
...
THUMB16 NEWCODE void Time_OnLayout(DISP_OBJ *disp, void *layoutstruct)
{
    DispObject_SetLayerColor(disp, clEmpty);
}

THUMB16 NEWCODE void Time_OnKey(DISP_OBJ *disp, int key, int count, int repeat, int mode)
{
    DISP_OBJ_TIMEINPUT *disp_obj =(DISP_OBJ_TIMEINPUT *)disp;
...
THUMB16 NEWCODE void Time_create(DISP_DESC *desc)
{
...
THUMB16 NEWCODE void Time_destroy(GUI_TIMEINPUT *gui)
{
}

THUMB16 NEWCODE void Time_CallBack(DISP_OBJ *, void *msg, GUI *)
{
}

THUMB16 NEWCODE void SetTimeInput(DBP_DATA *data, GUI_TIMEINPUT *gui)
{
...
THUMB16 NEWCODE void Time_OnSelect(BOOK *book, GUI *gui)
{
    DISP_OBJ_TIMEINPUT *disp_obj = (DISP_OBJ_TIMEINPUT *)GUIObject_GetDispObject(gui);
...
THUMB16 NEWCODE void OnBackTime(BOOK *book, GUI *gui)
{
    DBP_DATA *data = GetData();
    FREE_GUI(data->time_input);
}

THUMB16 NEWCODE void DBPlayer_Time(BOOK *book, GUI *gui)
{
    ...
    GUIObject_SetStyle(time_input, data->settings.screen ? UI_OverlayStyle_TrueFullScreen : UI_OverlayStyle_Default);
