...
extern "C" THUMB16 NEWCODE int 
dll_Disp_GetTextIDWidth(int font, TEXTID text_id, int len)
{
    ITextRenderingManager *mgr = 0;
    ITextRenderingFactory *factory = 0;
    IRichTextLayout *layout = 0;
    ...
    if (CoCreateInstance(CID_CTextRenderingManager,
                         IID_ITextRenderingManager,
                         PPINTERFACE(&mgr)) < 0 ||
        !mgr)
        return 0;
...
// GC_PutChar ----------------------------------------------------
extern "C" THUMB16 NEWCODE int Get_IUIImageManager(IUIImageManager **ppMgr)
{
    return CoCreateInstance(CID_CUIImageManager, IID_IUIImageManager,
                            PPINTERFACE(ppMgr));
}

extern "C" THUMB16 NEWCODE void dll_GC_PutChar(GC *gc, int x, int y,
...
extern "C" THUMB16 NEWCODE METADATA_DESC *
dll_MetaData_Desc_Create(wchar_t *path, wchar_t *name)
{
    METADATA_DESC *desc =
        (METADATA_DESC *)malloc(sizeof(METADATA_DESC));
    ...
    if (CoCreateInstance(CID_CMetaData,
                         IID_IMetaData,
                         PPINTERFACE(&desc->pIMetaData)) < 0 ||
        !desc->pIMetaData)
    {
        mfree(desc);
        return 0;
...
extern "C" THUMB16 NEWCODE void
dll_MetaData_Desc_Destroy(METADATA_DESC *desc)
{
    if (!desc)
        return;
...
extern "C" THUMB16 NEWCODE BOOL
dll_MetaData_Desc_HasAlbumArt(METADATA_DESC *desc,
                              wchar_t *path,
                              wchar_t *name)
{
...
extern "C" THUMB16 NEWCODE BOOL
dll_MetaData_Desc_GetCoverInfo(METADATA_DESC *desc,
                               char *cover_type,
                               int *size,
                               int *cover_offset)
{
