...
THUMB16
NEWCODE void FreeImage(DBP_DATA *data)
{
	char error_code;
	...
	for (int i = IMG_BACKGROUND; i < IMG_LAST; i++)
	{
		if (data->Image[i].ID != NOIMAGE)
			REQUEST_IMAGEHANDLER_INTERNAL_UNREGISTER(
			        &SYNC, data->Image[i].Handle, 0, 0, data->Image[i].ID, 1, &error_code);
	}
	...
	if (FSX_IsFileExists(fpath, fname))
	{
		...
		if (!REQUEST_IMAGEHANDLER_INTERNAL_GETHANDLE(&SYNC, &img->Handle, &error_code))
			if (!REQUEST_IMAGEHANDLER_INTERNAL_REGISTER(
			            &SYNC, img->Handle, fpath, fname, 0, &img->ID, &error_code))
				if (error_code)
					img->Handle = NOIMAGE;
	}
}
...
THUMB16
NEWCODE void InitIcon(DBP_DATA *Data)
{
	for (int i = IMG_BACKGROUND; i < IMG_LAST; i++)
	{
...
THUMB16
NEWCODE int TMMETime_to_Seconds(TMMETime Time)
{
	return ((Time.Hours * 3600) + (Time.Minutes * 60) + Time.Seconds);
}

THUMB16
NEWCODE void ResetPlayer(DBP_DATA *data)
{
	TMMETime time;
	time.Hours = 0;
	time.Minutes = 0;
...
THUMB16
NEWCODE void DBPlayer_DrawLyricViewer(DISP_OBJ_DBP *disp_obj)
{
	if (!disp_obj->lrclist)
	{
	...
	else if (disp_obj->current_offset >= READY)
	{
		...
		{
			text_ids[disp_obj->current_offset] = TextID_Create(
			        disp_obj->lrclist[disp_obj->current_offset].lrcinfo, ENC_UCS2, TEXTID_ANY_LEN);
			DrawTextEx(FONT_E_16B,
			           ...
			           disp_obj->disp_height,
			           clYellow);
			int width2 = TextID_GetWidth(FONT_E_16B,
			                             text_ids[disp_obj->current_offset],
			                             TextID_GetLength(text_ids[disp_obj->current_offset]));
...
THUMB16
NEWCODE void DBP_GetLyric(GUI_DBPLAYER *dbp, wchar_t *path, wchar_t *name)
{
...
THUMB16
NEWCODE int DBPlayer_onCreate(DISP_OBJ *disp)
{
...
THUMB16
NEWCODE void DBPlayer_onClose(DISP_OBJ *disp)
{
	DISP_OBJ_DBP *disp_obj = (DISP_OBJ_DBP *)disp;
...
THUMB16
NEWCODE void DBPlayer_DrawNowPlaying(DBP_DATA *data, DISP_OBJ_DBP *disp_obj)
{
	if (data->settings.artist_icn.state)
		DrawImage(data->settings.artist_icn.pos.x, data->settings.artist_icn.pos.y, data->Image[IMG_ARTIST].ID);

	if (data->settings.title_icn.state)
		DrawImage(data->settings.title_icn.pos.x, data->settings.title_icn.pos.y, data->Image[IMG_TITLE].ID);

	if (data->settings.album_icn.state)
		DrawImage(data->settings.album_icn.pos.x, data->settings.album_icn.pos.y, data->Image[IMG_ALBUM].ID);
...
THUMB16
NEWCODE void DBPlayer_onRedraw(DISP_OBJ *disp, int r1, int r2, int r3)
{
	DISP_OBJ_DBP *disp_obj = (DISP_OBJ_DBP *)disp;
	...
	case BG_TYPE_IMAGE:
		...
		break;
	case BG_TYPE_THEME: DispObject_SetThemeImage(disp, 0x1E); break;
	case BG_TYPE_COLOR:
		DrawRect(0,
		         0,
		         disp_obj->disp_width,
	...
	if (data->settings.frame.state) // Frame
		DrawImage(data->settings.frame.pos.x, data->settings.frame.pos.y, data->Image[IMG_FRAME].ID);
	...
	if (data->edit_visual)
	{
		if (data->edit_text_mode)
		{
			...
			DrawRect(data->temp.x1,
			         data->temp.y1,
			         data->temp.x2,
			         data->temp.y1 + FONTHEIGHT(data->temp.font),
			         clBlack,
			         0x35FFFFFF);
		}
		...
		DrawRect(0,
		         data->settings.screen ? ypos_rect
		                               : (ypos_rect - disp_obj->status_height - disp_obj->softkeys_height),
		         disp_obj->disp_width,
		...
		DrawTextWithStyle(disp_obj->buf_text,
		                  ...
		                  data->settings.screen
		                          ? ypos_coord
		                          : (ypos_coord - disp_obj->status_height - disp_obj->softkeys_height),
...
THUMB16
NEWCODE void DBPlayer_onRefresh(DISP_OBJ *disp)
{
...
THUMB16
NEWCODE void DBPlayer_onLayout(DISP_OBJ *disp, void *layoutstruct)
{
	DispObject_SetLayerColor(disp, clTransGray);
}

THUMB16
NEWCODE void DBPlayer_onKey(DISP_OBJ *disp, int key, int count, int repeat, int mode)
{
	DISP_OBJ_DBP *disp_obj = (DISP_OBJ_DBP *)disp;
	...
	if (!data->edit_visual)
	{
		if (mode == KBD_LONG_PRESS && (disp_obj->tab == PLAYER_TAB || disp_obj->tab == LYRIC_TAB))
		{
			switch (key)
			{
			case KEY_RIGHT: IMME_StartFastForward(data->pIMMEPlayer); break;
			case KEY_LEFT: IMME_StartRewind(data->pIMMEPlayer); break;
			}
		}
		...
		else if (mode == KBD_SHORT_RELEASE && disp_obj->tab == LYRIC_TAB)
		{
			switch (key)
			{
			case KEY_DIGITAL_0: disp_obj->lrc_show_inactive = !disp_obj->lrc_show_inactive; break;
			case KEY_DIGITAL_5: disp_obj->lrc_is_centered = !disp_obj->lrc_is_centered; break;
			case KEY_STAR: disp_obj->lrc_show_tags = !disp_obj->lrc_show_tags; break;
			case KEY_LEFT: disp_obj->tab = PLAYER_TAB; break;
			}
		}
		else if (mode == KBD_SHORT_RELEASE && disp_obj->tab == PLAYER_TAB)
		{
			switch (key)
			{
			case KEY_LEFT: ResetPlayer(data); break;
			case KEY_RIGHT: disp_obj->tab = LYRIC_TAB; break;
			}
		}
		else
		{
...
THUMB16
NEWCODE void DBPlayer_onDescCreate(DISP_DESC *desc)
{
...
THUMB16
NEWCODE void DBPlayer_onDestroy(GUI *gui) {}

THUMB16
NEWCODE void DBPlayer_Pause(BOOK *book, GUI *gui)
{
	DBP_DATA *data = GetData();
...
THUMB16
NEWCODE void DBPlayer_Stop(BOOK *book, GUI *gui)
{
	BookObj_ReturnPage(book, PREVIOUS_EVENT);
}

THUMB16
NEWCODE void DBPlayer_Minimise(BOOK *book, GUI *gui)
{
...
THUMB16
NEWCODE void DBPlayer_LongClose(BOOK *book, GUI *gui)
{
	BookObj_ReturnPage(book, CANCEL_EVENT);
}

THUMB16
NEWCODE void DBPlayer_onCallBack(DISP_OBJ *, void *msg, GUI *) {}

THUMB16
NEWCODE GUI_DBPLAYER *CreateGUI(BOOK *book)
{
	DBP_DATA *data = GetData();

	GUI_DBPLAYER *gui_dbp = (GUI_DBPLAYER *)malloc(sizeof(GUI_DBPLAYER));

	if (!GUIObject_Create(gui_dbp,
	                      DBPlayer_onDestroy,
	                      DBPlayer_onDescCreate,
	                      book,
	                      DBPlayer_onCallBack,
	                      UIDisplay_Main,
	                      sizeof(GUI_DBPLAYER)))
...
#ifdef DB3350
THUMB16
NEWCODE int Get_MME_DATA(void *pMMEData, BOOK *book)
{
...
THUMB16
NEWCODE void CreateInfo(BOOK *book)
{
	BrowserItemBook *itembook = (BrowserItemBook *)book;
...
#else
THUMB16
NEWCODE void CreateInfo(MME_DATA *pMMEData, BOOK *book)
{
	BrowserItemBook *itembook = (BrowserItemBook *)book;
