...
THUMB16 NEWCODE void RefreshList(SETTINGS_BOOK *setbook, int count)
{
...
THUMB16 NEWCODE void SetActivate(BOOK *book, int count)
{
...
THUMB16 NEWCODE void ProgressBar_OnSelect(BOOK *book, GUI *gui)
{
    DBP_DATA *data = GetData();
...
THUMB16 NEWCODE void Show_OnSelect(BOOK *book, GUI *gui)
{
    DBP_DATA *data = GetData();
...
THUMB16 NEWCODE void Align_OnSelect(BOOK *book, GUI *gui)
{
    SETTINGS_BOOK *setbook = (SETTINGS_BOOK *)book;
    DBP_DATA *data = GetData();
...
THUMB16 NEWCODE void OneOfMany_OnBack(BOOK *book, GUI *gui)
{
    SETTINGS_BOOK *setbook = (SETTINGS_BOOK *)book;
    FREE_GUI(setbook->options_menu);
}

THUMB16 NEWCODE void OneOfMany_SetMode(BOOK *book, int type)
{
    SETTINGS_BOOK *setbook = (SETTINGS_BOOK *)book;
    DBP_DATA *data = GetData();
...
THUMB16 NEWCODE int Color_OnCreate(DISP_OBJ *disp)
{
...
THUMB16 NEWCODE void Color_OnClose(DISP_OBJ *disp)
{
    DISP_OBJ_COLOR *disp_obj = (DISP_OBJ_COLOR *)disp;
...
THUMB16 NEWCODE void Color_OnRedraw(DISP_OBJ *disp, int, int, int)
{
    DISP_OBJ_COLOR *disp_obj = (DISP_OBJ_COLOR *)disp;
    ...
    for (int i = 0; i != 4; i++)
    {
        ...
        if (disp_obj->current_column == i)
            DrawRect(x1 + start_column - 2, y1 + f_size - 2 + 50, x1 + start_column + column_width + 2, y1 + f_size + column_height + 2, clBlack, clWhite);
        DrawRect(x1 + start_column, y1 + f_size + 50, x1 + start_column + column_width, y1 + f_size + column_height, clBlack, colors[i]);
        column_height--;
...
THUMB16 NEWCODE void Color_OnKey(DISP_OBJ *disp, int key, int, int repeat, int mode)
{
    DISP_OBJ_COLOR *disp_obj = (DISP_OBJ_COLOR *)disp;
...
THUMB16 NEWCODE void Color_create(DISP_DESC *desc)
{
...
THUMB16 NEWCODE void Color_destroy(GUI *gui)
{
}

THUMB16 NEWCODE void Color_OnSelect(BOOK *book, GUI *gui)
{
    SETTINGS_BOOK *setbook = (SETTINGS_BOOK *)book;
...
THUMB16 NEWCODE void Color_OnBack(BOOK *book, GUI *gui)
{
    SETTINGS_BOOK *setbook = (SETTINGS_BOOK *)book;
    FREE_GUI(setbook->color_picker);
}

THUMB16 NEWCODE void Color_CallBack(DISP_OBJ *, void *msg, GUI *)
{
}

THUMB16 NEWCODE void SetColor(BOOK *book, int type)
{
    SETTINGS_BOOK *setbook = (SETTINGS_BOOK *)book;
    GUI *gui_color = (GUI *)malloc(sizeof(GUI));
    if (!GUIObject_Create(gui_color, Color_destroy, Color_create, &setbook->book, Color_CallBack, UIDisplay_Main, sizeof(GUI)))
    {
        mfree(gui_color);
        return;
    }
...
THUMB16 NEWCODE void SetActiveSoftkeys(BOOL mode)
{
    DBP_DATA *data = GetData();
...
THUMB16 NEWCODE const wchar_t *Font_GetNameByFontId(int font_size)
{
#if defined(DB3200) || defined(DB3210) || defined(DB3350)
    ...
    case UIFontStylePlain:
        snwprintf(font_name, MAXELEMS(font_name), FMT_FONT_R, font_size);
        break;
    case UIFontStyleBold:
        snwprintf(font_name, MAXELEMS(font_name), FMT_FONT_B, FONTHEIGHT(font_size));
        break;
    case UIFontStyleItalic:
        snwprintf(font_name, MAXELEMS(font_name), FMT_FONT_I, FONTHEIGHT(font_size));
        break;
...
THUMB16 NEWCODE int GetIdByFontId(int id)
{
    int ret = 0;
    FONT_DESC *font_desc = GET_FONTDESC_PTR;
    int total_font = *GET_FONTCOUNT_PTR;
...
THUMB16 NEWCODE void SetVisual(BOOK *book)
{
    SETTINGS_BOOK *setbook = (SETTINGS_BOOK *)book;
    DBP_DATA *data = GetData();
