...
THUMB16
NEWCODE void TimerList_Free(TimerList *lrclist)
{
	int list_num;
...
THUMB16
NEWCODE void Lyric_FreeData(DISP_OBJ_DBP *disp_obj)
{
	if (disp_obj->lrclist)
	{
...
THUMB16
NEWCODE int find_current_timer_list(DISP_OBJ_DBP *disp_obj, time_t tm)
{
	int i = 0;
	while (i <= disp_obj->total_offset)
	{
		...
		{
			if ((tm - disp_obj->lrclist[i + 1].timer) <= 100 ||
			    (disp_obj->lrclist[i + 1].timer - tm <= 100))
			{
				return i + 1;
			}
			else if ((tm - disp_obj->lrclist[i + 2].timer) <= 100 ||
			         (disp_obj->lrclist[i + 2].timer - tm <= 100))
			{
				return i + 2;
			}
			else
...
THUMB16
NEWCODE int str_to_timer(char *str)
{
	int len = strlen(str);
	int i = 0;
	int ret = 0;
...
THUMB16
NEWCODE void sort_timer(DISP_OBJ_DBP *disp_obj)
{
	int i = 0;
	while (i <= disp_obj->total_offset)
	{
		...
		{
			if (disp_obj->lrclist[i].timer > disp_obj->lrclist[j].timer && disp_obj->lrclist[i].timer > 0 &&
			    disp_obj->lrclist[j].timer > 0)
			{
				TimerList tmp;
...
THUMB16
NEWCODE int songname_to_lrc(const wchar_t *songname, wchar_t *lrc, int len)
{
	if (!songname || !len)
		return 0;
...
THUMB16
NEWCODE int LoadLrc(DISP_OBJ_DBP *disp_obj, const wchar_t *path, const wchar_t *name)
{
	mfree(disp_obj->lrcbuf);
		...
		if (disp_obj->lrcbuf[i] == '[')
		{
			if (is_digit(disp_obj->lrcbuf[i + 1]))
			{
				...
				if (strstr(tmp, "."))
				{
					strncpy(ss,
					        strstr(tmp, ":") + 1,
					        strlen(strstr(tmp, ":")) - strlen(strstr(tmp, ".")) - 1);
					strncpy(ms, strstr(tmp, ".") + 1, strlen(strstr(tmp, ".")) - 1);
				}
				else
					strncpy(ss, strstr(tmp, ":") + 1, strlen(strstr(tmp, ":")) - 1);
				disp_obj->lrclist[disp_obj->total_offset].timer =
				        str_to_timer(ms) + str_to_timer(ss) * 1000 + str_to_timer(mm) * 1000 * 60;
...
THUMB16
NEWCODE void GetLyric(DISP_OBJ_DBP *disp_obj, const wchar_t *path, const wchar_t *name)
{
...
THUMB16
NEWCODE void lyricOnTimer(uint16_t timerID, LPARAM disp)
{
	...
	int betweentimer = disp_obj->lrclist[disp_obj->current_offset + 1].timer -
	                   disp_obj->lrclist[disp_obj->current_offset].timer;
...
THUMB16
NEWCODE void Start_LyricTimer(DISP_OBJ_DBP *disp_obj)
{
	disp_obj->lrc_timer_id = Timer_Set(0, lyricOnTimer, (LPARAM)disp_obj);
}

THUMB16
NEWCODE void Kill_LyricTimer(DISP_OBJ_DBP *disp_obj)
{
