...
THUMB16
NEWCODE int TextID_GetWidth(int font, TEXTID text_id, int len)
{
...
THUMB16
NEWCODE int encoding_get_type(char *str)
{
...
THUMB16
NEWCODE char *unicode2win1251(char *s, wchar_t *ws, size_t len)
{
	char *d = s;
	int c;
	while ((c = *ws++) && ((len--) > 0))
...
THUMB16
NEWCODE void win12512unicode(wchar_t *ws, const char *s, size_t len)
{
	int c;
	while ((c = *s++) && ((len--) > 0))
	{
...
THUMB16
NEWCODE void utf8_to_ansi(char *ansi, const char *utf8, size_t max_len)
{
	size_t i = 0;
	size_t j = 0;
...
THUMB16
NEWCODE void utf16le_to_ansi(const wchar_t *utf16, char *ansi, size_t max_len)
{
	size_t i = 0;
	size_t j = 0;
...
THUMB16
NEWCODE void utf16be_to_ansi(const wchar_t *utf16be, char *ansi, size_t max_len)
{
	size_t i = 0;
	size_t j = 0;
...
THUMB16
NEWCODE BOOL is_digit(char c)
{
	return (c >= '0' && c <= '9');
}

THUMB16
NEWCODE char *str_alloc(int lenght)
{
...
THUMB16
NEWCODE wchar_t *wstr_alloc(int lenght)
{
