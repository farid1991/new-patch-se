...
THUMB16 NEWCODE static inline void mfree(void *mem)
{
    if (mem)
...
THUMB16 NEWCODE vdata_t *Get_VDATA()
{
...
THUMB16 NEWCODE BOOL is_java_running(BOOK *book)
{
    char str[100];
...
THUMB16 NEWCODE wchar_t *GetCurrentSkinPath(wchar_t *out)
{
    wstrcpy(out, DIR_OTHER_PATH);

    int file = _fopen(DEFAULT_CFG_PATH, VOL_CFG,
                      FSX_O_RDONLY, FSX_S_IREAD, NULL);
...
THUMB16 NEWCODE int GetConfig(const wchar_t *skin_path, skin_config_t *sc)
{
...
THUMB16 NEWCODE void RegisterImage(image_t *img, const wchar_t *path, const wchar_t *fname)
{
    char err;
    img->id = NOIMAGE;
    img->handle = NOIMAGE;
...
THUMB16 NEWCODE void DrawProgressBar(GC *gc_ctx,
                                     IMAGEID blob_id,
                                     int value,
                                     int max_value,
                                     RECT rect,
...
THUMB16 NEWCODE void DrawText(int font, TEXTID text, int align, int xpos, int ypos, int width, uint32_t color)
{
    if (text != EMPTY_TEXTID)
    {
#if defined(DB3200) || defined(DB3210)
        dll_DrawString(font, text, align, xpos,
                       ypos, xpos + width, ypos + (font & 0xFF),
                       color);
#else
        SetFont(font);
        DrawString(text, align,
                   xpos, ypos, xpos + width, ypos + (font & 0xFF),
                   0, 1,
                   color, clEmpty);
#endif
    }
...
THUMB16 NEWCODE int New_Volume_OnCreate(DISP_OBJ *disp_obj)
{
...
THUMB16 NEWCODE int New_Volume_OnEnter(void *r0, BOOK *book)
{
