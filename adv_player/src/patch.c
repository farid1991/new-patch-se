...
static const char SK_SKINS_TXT[] = "Skins";
static const char SK_COVER_TXT[] = "Save coverart";
static const char SK_TAGGER_TXT[] = "Edit Tag";
static const char SK_MUSIC_TXT[] = "My Music";
static const char SK_INFO_MUSIC_TXT[] = "Browse Artists,Albums,Tracks,Playlists,Videos";
static const char SK_INFO_SKINS_TXT[] = "Change skin of WALKMAN display";

static const char ADV_MEM[] = "ADV_MEM";

THUMB16
NEWCODE void *malloc(int size)
{
#if defined(DB2010)
	return memalloc(size, 1, 5, ADV_MEM, 0);
#elif defined(DB2020)
	return memalloc(0, size, 1, 5, ADV_MEM, 0);
#endif
}
...
NEWCODE void mfree(void *mem)
{
	if (mem)
#if defined(DB2010)
		memfree(mem, ADV_MEM, 0);
#elif defined(DB2020)
		memfree(0, mem, ADV_MEM, 0);
#endif
}

#ifdef OLD_PLAYER
THUMB16
NEWCODE BOOL _FSX_IsFileExists(const wchar_t *fpath, const wchar_t *fname)
{
	FSTAT fs;
...
THUMB16
NEWCODE TEXTID conv_sample_rate(int freq)
{
	wchar_t freq_buffer[11];
...
THUMB16
NEWCODE void New_MediaPlayer_Audio_OnRedraw(DISP_OBJ *disp_obj, int a, RECT *window, RECT *c)
{
...
NEWCODE void New_MediaPlayer_PlayQueue_SetStyle(DISP_OBJ *disp_obj, RECT *rc)
{
	...
	DispObject_SetListTextColor(
	        disp_obj, 0, data->skin_data->PL_item_color, 0, 0, data->skin_data->PL_highlight_color, 0, 0);
#endif
}

THUMB16
NEWCODE void New_MediaPlayer_PlayQueue_SetTitle(DISP_OBJ *disp_obj)
{
	ADVPLAYER_DATA *data = env_data_get();
...
THUMB16
NEWCODE int New_MediaPlayer_NowPlaying_OnCreate(DISP_OBJ_NOWPLAYING *disp_obj)
{
...
THUMB16
NEWCODE void New_MediaPlayer_NowPlaying_OnRedraw(DISP_OBJ_NOWPLAYING *disp_obj, RECT *a, RECT *b, RECT *c)
{
	ADVPLAYER_DATA *data = env_data_get();
	...
	if (data->skin_data->bg_enable)
		draw_image(disp_gc, data->skin_data->bg_x, data->skin_data->bg_y, data->skin_images[BACKGROUND_ICN].id);
	...
	if (data->skin_data->coverart_enable)
	{
		if (data->has_coverart)
		{
			if (data->cv_gvi == 0)
			{
				...
				data->canvas_width = (cv_width <= RESIZED_CANVAS && cv_height <= RESIZED_CANVAS)
				                             ? cv_width
				                             : RESIZED_CANVAS;
				data->canvas_height = (cv_width <= RESIZED_CANVAS && cv_height <= RESIZED_CANVAS)
				                              ? cv_height
				                              : RESIZED_CANVAS;

				data->cv_canvas =
				        GC_CreateMemoryGC(data->canvas_width, data->canvas_height, 16, 0, 0, 0);
				CANVAS_Get_GviGC(data->cv_canvas->pcanvas, &data->cv_gvi);

				GVI_BRUSH backbrush = GVI_CreateSolidBrush(clBlack);
				GVI_FillRectangle(
				        data->cv_gvi, 0, 0, data->canvas_width, data->canvas_height, backbrush);
				GVI_Delete_GVI_Object(&backbrush);

				draw_image_ex(data->cv_canvas,
				              0,
				              0,
				              data->canvas_width,
				              data->canvas_height,
			...
			GVI_StretchBlt(data->disp_gvi,
			               data->skin_data->coverart_x,
			               data->skin_data->coverart_y,
			               data->skin_data->coverart_w,
			               data->skin_data->coverart_h,
			               data->cv_gvi,
			               0,
			               0,
			               data->canvas_width,
	...
	{
		draw_image(disp_gc,
		           data->skin_data->overlay_x,
		           data->skin_data->overlay_y,
	...
	{
		draw_image(disp_gc,
		           data->skin_data->icon_walkman_x,
		           data->skin_data->icon_walkman_y,
	...
	{
		draw_image(disp_gc,
		           data->skin_data->icon_artist_x,
		           data->skin_data->icon_artist_y,
	...
	{
		draw_image(disp_gc,
		           data->skin_data->icon_title_x,
		           data->skin_data->icon_title_y,
	...
	{
		draw_image(disp_gc,
		           data->skin_data->icon_album_x,
		           data->skin_data->icon_album_y,
		           data->skin_images[MP_TITLE_ICN].id);
	}
	...
	if (data->skin_data->icon_next_enable)
	{
		if (data->player_state == TMusicState_FastForwarding)
			draw_image(disp_gc,
			           data->skin_data->icon_next_x,
			           data->skin_data->icon_next_y,
	...
	if (data->skin_data->icon_prev_enable)
	{
		if (data->player_state == TMusicState_Rewinding)
			draw_image(disp_gc,
			           data->skin_data->icon_prev_x,
			           data->skin_data->icon_prev_y,
	...
	if (data->skin_data->icon_playerstate_enable)
	{
		if (data->is_playing)
			draw_image(disp_gc,
			           data->skin_data->icon_playerstate_x,
			           data->skin_data->icon_playerstate_y,
	...
	{
		draw_image(disp_gc,
		           data->skin_data->icon_random_x,
		           data->skin_data->icon_random_y,
	...
	{
		draw_image(disp_gc,
		           data->skin_data->icon_repeat_x,
		           data->skin_data->icon_repeat_y,
	...
	if (data->skin_data->icon_eq_enable)
	{
		...
		case EqPreset_Normal:
			draw_image(disp_gc,
			           data->skin_data->icon_eq_x,
			           data->skin_data->icon_eq_y,
			           data->skin_images[MP_EQ_NORMAL_ICN].id);
			break;
		case EqPreset_Bass:
			draw_image(disp_gc,
			           data->skin_data->icon_eq_x,
			           data->skin_data->icon_eq_y,
			           data->skin_images[MP_EQ_BASS_ICN].id);
			break;
#ifdef MEGABASS
		case EqPreset_Megabass:
			draw_image(disp_gc,
			           data->skin_data->icon_eq_x,
			           data->skin_data->icon_eq_y,
			           data->skin_images[MP_EQ_MEGABASS_ICN].id);
			break;
#endif
		case EqPreset_TrebleBoost:
			draw_image(disp_gc,
			           data->skin_data->icon_eq_x,
			           data->skin_data->icon_eq_y,
			           data->skin_images[MP_EQ_TREBLEBOOST_ICN].id);
			break;
		case EqPreset_Voice:
			draw_image(disp_gc,
			           data->skin_data->icon_eq_x,
			           data->skin_data->icon_eq_y,
			           data->skin_images[MP_EQ_VOICE_ICN].id);
			break;
		default:
			draw_image(disp_gc,
			           data->skin_data->icon_eq_x,
			           data->skin_data->icon_eq_y,
			           data->skin_images[MP_EQ_MANUAL_ICN].id);
			break;
		}
	}
...
THUMB16
NEWCODE void GetTrackInfo(AudioPlayerBook *audio_book)
{
	ADVPLAYER_DATA *data = env_data_get();
...
THUMB16
NEWCODE void GetPlayerState(AudioPlayerBook *audio_book)
{
	ADVPLAYER_DATA *data = env_data_get();
...
THUMB16
NEWCODE void save_coverart(BOOK *book, GUI *gui)
{
	AudioPlayerBook *audio_book = (AudioPlayerBook *)book;
...
THUMB16
NEWCODE void enter_skinselector(BOOK *audio_book, GUI *gui)
{
	BookObj_CallPage(audio_book, &page_MediaPlayer_SkinSelectorPage);
}

THUMB16
NEWCODE void enter_mymusic(BOOK *audio_book, GUI *gui)
{
	UI_Event(MM_BRW_TOPLEVEL_STARTED_EVENT);
}

THUMB16
NEWCODE TEXTID new_softkeys_text(uint8_t type)
{
	if (type == 1)
		return TextID_Create(SK_SKINS_TXT, ENC_LAT1, MAXELEMS(SK_SKINS_TXT));
	else if (type == 2)
		return TextID_Create(SK_COVER_TXT, ENC_LAT1, MAXELEMS(SK_COVER_TXT));
	else if (type == 3)
		return TextID_Create(SK_TAGGER_TXT, ENC_LAT1, MAXELEMS(SK_TAGGER_TXT));
	return TextID_Create(SK_MUSIC_TXT, ENC_LAT1, MAXELEMS(SK_MUSIC_TXT));
}

THUMB16
NEWCODE TEXTID new_softkeys_infotext(uint8_t type)
{
	if (type == 0)
		return TextID_Create(SK_INFO_MUSIC_TXT, ENC_LAT1, MAXELEMS(SK_INFO_MUSIC_TXT));
	return TextID_Create(SK_INFO_SKINS_TXT, ENC_LAT1, MAXELEMS(SK_INFO_SKINS_TXT));
}

THUMB16
NEWCODE void New_MediaPlayer_SoftKeys_MyMusic(AudioPlayerBook *audio_book)
{
	...
	if (MediaPlayer_SoftKeys_Validate(audio_book->Gui_NowPlaying))
		MediaPlayer_SoftKeys_AddHelpStr(
		        audio_book->Gui_NowPlaying, MP_ACTION_MYMUSIC, new_softkeys_infotext(0));
	else
		GUIObject_SoftKeys_SetInfoText(audio_book->Gui_NowPlaying, MP_ACTION_MYMUSIC, new_softkeys_infotext(0));
}

THUMB16
NEWCODE void New_MediaPlayer_SoftKeys(AudioPlayerBook *audio_book)
{
