...
THUMB16 NEWCODE uint32_t id3_header_get_tag_size(uint8_t *data, int byte_count)
{
...
THUMB16 NEWCODE uint32_t id3_frame_get_size(uint8_t *data, uint32_t fpos, int version)
{
    uint32_t res = 0;
    if (version == ID3V2_4)
    {
...
static inline BOOL is_valid_id3_header(uint32_t header)
{
...
THUMB16 NEWCODE int get_bitrate(uint32_t header)
{
...
THUMB16 NEWCODE int get_frequency(uint32_t header)
{
    int version_id = get_version_id(header);           // MPEG version ID
...
THUMB16 NEWCODE const char *get_mode(uint32_t header)
{
...
THUMB16 NEWCODE uint32_t id3v2_read_header(const wchar_t *path, const wchar_t *name, uint32_t *tagsize, uint32_t *version, uint32_t *fpos)
{
    ...
    *fpos = 0;
    if (!memcmp(header_buf, id3_hdr_id, ID3_HEADER_ID_LENGTH))
    {
        *version = header_buf[3];
        ...
        lseek(file, *tagsize + ID3_HEADER_SIZE, FSX_SEEK_SET);
    }
    else
    {
        fclose(file);
